...
A dedicated subsystem manages default days per city for custom tour calculations. Admins set days count per city via a two-tab interface (Components | Cities) within each country section of the admin panel. The system enforces unique (countryId, cityId) pairs and validates daysCount > 0 at database level. When users create custom tour orders, the frontend auto-fetches city-days data, calculates and displays total tour duration based on selected cities, and submits totalDays field with the order. This eliminates manual day calculations and ensures consistency across custom tour requests.
