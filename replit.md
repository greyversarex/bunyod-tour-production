...
-   **Booking & Order System**: Seamless flow from draft bookings to payment-ready orders, including a 3-step localized booking process with unified state management and debounced price recalculations. **Fixed critical price calculation bug** in Step 2 (`updateBookingDetails`) - now correctly handles both simple `{ SGL: 1 }` and full `{ SGL: { quantity: 1, price: 100 } }` roomSelection formats, retrieves stored selections from database when not provided in request, and preserves hotel/meal selections across step transitions without data loss.
