...
-   **Critical Fix (Oct 21, 2025)**: Resolved HTTP 400 errors when saving tours from admin panel by implementing bidirectional enum normalization (frontend maps Russian labels to English enum values before API submission; backend defensively normalizes incoming values for backward compatibility). Fixed logic ensures partial updates don't overwrite existing priceType/tourType with defaults.
