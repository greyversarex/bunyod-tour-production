...
-   **Custom Tour Direct Payment Integration**: Direct payment model for custom tour orders; tourists create orders and pay immediately without admin approval. Features server-side component price validation, order data stored in Order.wishes field as JSON, orderNumber format `CT-{timestamp}-{customerId}`, payment gateway revalidation, and admin email notifications.
