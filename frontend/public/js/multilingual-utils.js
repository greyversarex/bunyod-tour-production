...
function stripHtmlTagsForCards(html) {
    if (!html) return '';
    return String(html)
        .replace(/<[^>]*>/g, '')
        .replace(/&nbsp;/g, ' ')
 ...
 */
function parseMultilingualField(jsonField, language = 'en') {
...
function updateTourDescriptions(language) {
  document.querySelectorAll('[data-tour-description]').forEach(element => {
    ...
    // Strip HTML tags for card descriptions
    try {
      const parsed = typeof descData === 'string' ? JSON.parse(descData) : descData;
      const text = parsed[language] || parsed.ru || parsed.en || '';
      element.textContent = stripHtmlTagsForCards(text);
    } catch (e) {
...
function getCurrentLanguage() {
  // Приоритет: window.currentLanguage > localStorage > default 'en'
  // Английский — основной язык по умолчанию (фундаментальное правило).
