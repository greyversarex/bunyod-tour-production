...
const bookingStateManager = {
    ...
    init() {
        ...
        if (stored && timestamp) {
            ...
            if (age < STATE_EXPIRY_MS) {
                try {
                    ...
                    // 🌍 КРИТИЧНО: принудительно синхронизируем язык с глобальным
                    // window.currentLanguage (единый источник правды из i18n.js).
                    // Защита от случая, когда в sessionStorage остался старый
                    // language: 'ru' от предыдущего бронирования, а пользователь
                    // уже переключился на 'en' через language selector в header.
                    // Без этой синхронизации booking-flow рендерился бы на старом
                    // языке, пока остальные страницы — на новом.
                    if (window.currentLanguage && this.state.language !== window.currentLanguage) {
                        console.log(`🔄 Syncing booking language: ${this.state.language} → ${window.currentLanguage}`);
                        this.state.language = window.currentLanguage;
                        this.persist();
                    }
        ...
        // Свежий state: тоже синхронизируем язык с глобальным значением
    ...
    async syncWithServer() {
        ...
        try {
            const lang = window.currentLanguage || this.state.language || 'en';
