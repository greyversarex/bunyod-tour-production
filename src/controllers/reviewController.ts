...
// 🚀 Кэш статистики отзывов по туру: агрегации тяжёлые, а запрашиваются на каждой карточке/странице тура.
const REVIEW_STATS_TTL = 5 * 60 * 1000;
const reviewStatsKey = (tourId: number) => `review_stats_${tourId}`;
// Отзывы пишут/модерируют редко — при любом изменении сбрасываем всю статистику.
const clearReviewStatsCache = () => tourCache.clearPattern('review_stats');
...
export const moderateReview = async (req: Request, res: Response) => {
  try {
    ...
    // Статистика отзывов изменилась — сбрасываем кэш
    clearReviewStatsCache();
...
export const getReviewStats = async (req: Request, res: Response) => {
  try {
    const tourIdNum = parseInt(req.params.tourId);

    const cacheKey = reviewStatsKey(tourIdNum);
    const cached = tourCache.get(cacheKey);
    if (cached !== undefined) {
      return res.json({ success: true, data: cached });
    }

    // Обе агрегации независимы — выполняем ПАРАЛЛЕЛЬНО (раньше шли последовательно → 2x задержка).
    ...
    tourCache.set(cacheKey, data, REVIEW_STATS_TTL);
