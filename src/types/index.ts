...
export interface CreateTransferRequestData {
  ...
  dropoffLocation: string;
  pickupTime: string;       // "HH:MM" format
  pickupDate: string;       // "YYYY-MM-DD" format — дата начала аренды
  dropoffDate?: string;     // "YYYY-MM-DD" format — дата окончания аренды
  rentalDays?: number;      // Количество дней аренды
  numberOfPeople?: number;
  vehicleType?: 'sedan' | 'suv' | 'minibus' | 'bus' | string;
  vehicleCapacity?: string; // Выбранная вместимость в фильтрах
  vehicleId?: number;       // ID конкретного транспортного средства
  countryId?: number;       // ID страны
  cityId?: number;          // ID города
  specialRequests?: string;
  estimatedPrice?: number;  // Базовая цена за 1 день
