...
// Local credential sidecar that issues short-lived tokens for the object storage bucket.
const STORAGE_SIDECAR_ENDPOINT =
  process.env.STORAGE_SIDECAR_ENDPOINT || "http://127.0.0.1:1106";
const STORAGE_AUDIENCE = process.env.STORAGE_AUDIENCE || "object-storage";
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to sign object URL, errorcode: ${response.status}, ` +
        `make sure the storage sidecar is reachable at ${STORAGE_SIDECAR_ENDPOINT}`
    );
  }
