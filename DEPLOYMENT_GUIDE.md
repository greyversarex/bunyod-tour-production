...
> Все примеры и скрипты в этом руководстве используют этот путь.
...
- Проверьте ошибки: `npm run typecheck`
- Убедитесь что dist/ создалась на сервере

---

## 13. 🔧 Troubleshooting: TypeScript Компиляция

### Проблема: Сервер не запускается после обновления

**Симптомы:**
- `curl /healthz` возвращает код 000
- PM2 показывает ошибки в логах
...
# 2. Проверить что папка dist/ существует
ls -la /srv/bunyod-tour/dist/

# 3. Проверить режим запуска
echo $NODE_ENV
```

**Решение:**

**Вариант A: Пересобрать TypeScript вручную**
```bash
cd /srv/bunyod-tour
npm run build
...
**Вариант B: Использовать update.sh (рекомендуется)**
```bash
cd /srv/bunyod-tour
./update.sh
```

**Вариант C: Проверить TypeScript ошибки**
```bash
# Проверить типы без компиляции
npm run typecheck

# Если есть ошибки - исправить и пересобрать
npm run build
```

### Проблема: API endpoints возвращают 404

**Причина:** TypeScript код не скомпилирован в production

**Проверка:**
```bash
# Убедиться что NODE_ENV=production установлен
pm2 env 0 | grep NODE_ENV

# Проверить что dist/routes/ существует
ls /srv/bunyod-tour/dist/routes/
```
...
- Использует `ts-node` для компиляции на лету
- Загружает файлы из `./src/**/*.ts`
- Медленнее, но удобно для разработки
- Запуск: `npm start` (без NODE_ENV=production)

**Production Mode (PROD):**
- Использует предварительно скомпилированный код
- Загружает файлы из `./dist/**/*.js`
- Быстрее и стабильнее
- Запуск: `NODE_ENV=production npm start`

**Проверка текущего режима:**
Посмотрите на логи при старте сервера:
- DEV: `🛠️  DEV MODE: Using ts-node for TypeScript compilation`
- PROD: `🏭 PRODUCTION MODE: Using pre-compiled JavaScript from dist/`

### Автоматическая компиляция при обновлении

Скрипт `update.sh` теперь автоматически:
1. Делает `git pull`
2. Устанавливает зависимости (`npm ci`)
3. Генерирует Prisma Client (`npx prisma generate`)
4. **КОМПИЛИРУЕТ TypeScript** (`npm run build`) ← **НОВОЕ!**
5. Перезапускает PM2

**Если update.sh завершился с ошибкой:**
```bash
...
- [ ] **TypeScript скомпилирован (npm run build)** ← **НОВОЕ!**
- [ ] .env файл настроен с NODE_ENV=production
- [ ] Prisma схема применена (db push)
- [ ] Seed данные загружены
- [ ] Nginx настроен и запущен
- [ ] PM2 настроен и запущен (с NODE_ENV=production)
- [ ] SSL сертификат установлен
- [ ] Сайт открывается по HTTPS
- [ ] Админ-панель работает
- [ ] API endpoints работают (/api/health, /api/tour-blocks)
